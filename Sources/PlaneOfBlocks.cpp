...
#define SET_FUNCPTR(blksizex, blksizey, blksizex2, blksizey2)	do \
	{ \
		...
		VAR = Var##blksizex##x##blksizey##_sse2; \
		LUMA = Luma##blksizex##x##blksizey##_sse2; \
...
//#define NEWBLIT
	...
	#define SETBLIT16(blksizex, blksizey, type)	do \
		{ \
	...
	#define SETBLITX(blksizex, blksizey, type)	do \
		{ \
			type = copy_mc_##blksizex##x##blksizey##_mmx; \
		} while (false)
#else	// NEWBLIT
	#define SETBLIT16(blksizex, blksizey, type)	do { } while (false)
	#define SETBLITX(blksizex, blksizey, type)	do { } while (false)
#endif	// NEWBLIT
...
#define SET_FUNCPTR_x264(blksizex, blksizey, type)	do \
	{ \
		...
		if (cache64) type = x264_pixel_sad_##blksizex##x##blksizey##_cache64_mmx2; \
		...
		if (satd) type = x264_pixel_satd_##blksizex##x##blksizey##_mmx2; \
...
#define SET_FUNCPTR_x264_mmx(blksizex, blksizey, type) do \
	{ \
		type = x264_pixel_sad_##blksizex##x##blksizey##_mmx2; \
		if (cache32) type = x264_pixel_sad_##blksizex##x##blksizey##_cache32_mmx2; \
		if (cache64) type = x264_pixel_sad_##blksizex##x##blksizey##_cache64_mmx2; \
		if (ssd) type = x264_pixel_ssd_##blksizex##x##blksizey##_mmx; \
		if (satd) type = x264_pixel_satd_##blksizex##x##blksizey##_mmx2; \
		if (satd&&sse2) type = x264_pixel_satd_##blksizex##x##blksizey##_sse2; \
...
#define SET_FUNCPTR_x264_mmx_4x(blksizey, type) do \
	{ \
		type = x264_pixel_sad_4x##blksizey##_mmx2; \
		if (ssd) type = x264_pixel_ssd_4x##blksizey##_mmx; \
		if (satd) type = x264_pixel_satd_4x##blksizey##_mmx2; \
	} while (false)

#define SET_FUNCPTR_x264_SATD(blksizex,blksizey) do \
	{ \
		SATD = x264_pixel_satd_##blksizex##x##blksizey##_mmx2; \
	...
	if (0&&mmxext) //use new functions from x264
	{
		...
		default:
			...
			{ // 8x4
				...
				if (yRatioUV==2)
				{	//for making it obvious,that yv12 is empty
